...
VanillaAutoencoder, DeepCompress, VQVAE
"""
import torch
...
class VanillaAutoencoder(nn.Module):
    ...
    @property
    def compression_ratio(self): return self.n_points/self.latent_dim
...
class SpectralAttention(nn.Module):
    ...
    def forward(self,x):
        xt=x.permute(0,2,1); out,_=self.attn(xt,xt,xt)
...
class DeepCompress(nn.Module):
    def __init__(self,n_points=1000,latent_dim=10,dropout=0.1):
        ...
        with torch.no_grad():
            d=torch.zeros(1,1,n_points); co=self.enc_cnn(d)
            self._L=co.shape[2]; fd=64*self._L
    ...
    def encode(self,x):
        h=self.enc_cnn(x.unsqueeze(1)); h=self.enc_attn(h)
        return self.enc_proj(h.flatten(1))
    def decode(self,z):
        h=self.dec_proj(z).view(z.shape[0],64,self._L)
        h=self.dec_cnn(h).squeeze(1)
        if h.shape[1]>self.n_points: h=h[:,:self.n_points]
        elif h.shape[1]<self.n_points: h=F.pad(h,(0,self.n_points-h.shape[1]))
        return h
    def forward(self,x,temp=None): return self.decode(self.encode(x)),self.encode(x)
    @property
    def compression_ratio(self): return self.n_points/self.latent_dim
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self,z):
        d=(torch.sum(z**2,dim=1,keepdim=True)-2*z@self.embedding.weight.T
           +torch.sum(self.embedding.weight**2,dim=1))
        idx=torch.argmin(d,dim=1); zq=self.embedding(idx)
        loss=F.mse_loss(zq.detach(),z)+self.commitment_cost*F.mse_loss(zq,z.detach())
        zq=z+(zq-z).detach()
        return zq,idx,loss
...
class VQVAE(nn.Module):
    def __init__(self,n_points=1000,latent_dim=10,num_embeddings=512):
        super().__init__()
        self.n_points,self.latent_dim=n_points,latent_dim
        self.encoder=nn.Sequential(nn.Linear(n_points,512),nn.ReLU(),nn.Linear(512,256),nn.ReLU(),nn.Linear(256,latent_dim))
        self.vq=VectorQuantizer(num_embeddings,latent_dim)
        self.decoder=nn.Sequential(nn.Linear(latent_dim,256),nn.ReLU(),nn.Linear(256,512),nn.ReLU(),nn.Linear(512,n_points))
    def forward(self,x,temp=None):
        z=self.encoder(x); zq,idx,vql=self.vq(z); xh=self.decoder(zq)
        rl=F.mse_loss(xh,x)
        return xh,idx,{"loss_total":(rl+vql).item(),"loss_recon":rl.item(),"loss_vq":vql.item()}
    def encode_to_tokens(self,x):
        z=self.encoder(x); _,idx,_=self.vq(z); return idx
    @property
    def compression_ratio(self): return self.n_points/self.latent_dim
...
def train_baseline(model,train_loader,device,epochs=10,lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            out=model(x); loss=F.mse_loss(out[0],x)
            opt.zero_grad(); loss.backward(); opt.step()
            total+=loss.item(); n+=1
        if (epoch+1)%max(1,epochs//3)==0:
            print(f"    [{model.__class__.__name__}] epoch {epoch+1}/{epochs} loss={total/n:.4f}")
    return model

def eval_baseline(model,loader,device):
    model.eval(); all_hat,all_true=[],[]
    with torch.no_grad():
        for batch in loader:
            x=batch["spectrum"].to(device); out=model(x)
            all_hat.append(out[0].cpu().numpy()); all_true.append(x.cpu().numpy())
