...
# ──────────────────────────────────────────────────────────────────────────────
# Public API (called from evaluation/eval.py)
# ──────────────────────────────────────────────────────────────────────────────

def load_duke_dataset(path: str, n_points: int = 1000) -> Optional[Dict]:
    """
    Thin public wrapper around load_duke_edfa.

    Returns the dataset dict on success, or None if unavailable.
    eval.py uses None as the signal to fall back to a synthetic proxy.
    """
    try:
        return load_duke_edfa(local_path=path, download=True, n_points=n_points)
    except Exception as e:
        warnings.warn(f"[Duke] load_duke_dataset failed: {e}")
        return None


def make_synthetic_duke_proxy(n_samples: int = 1000,
                              n_points: int = 1000) -> Dict:
    """
    Return a synthetic Duke-calibrated dataset dict without touching disk.
    Used as a fallback inside eval.py when real data is unavailable.
    """
    sp, tp, wl = _proxy_spectra(n_samples, n_points)
    return {
        "spectra":     sp,
        "temperature": tp,
        "wavelengths": wl,
...
def run_zero_shot_eval(model, duke: Dict, stats: Dict, device) -> Dict:
    """
    Zero-shot evaluation: model trained only on synthetic data, tested on Duke.

    Parameters
    ----------
    model  : trained SpecCompress model
    duke   : dict with keys spectra [N,n_pts], temperature [N], wavelengths [n_pts]
    stats  : normalisation stats from training set
    device : torch.device

    Returns
    -------
    dict with key ``rmse_zero_shot``
    """
    import torch
    from evaluation.metrics import rmse_db

    model.eval()
    n    = min(len(duke["spectra"]), 2000)
    sp   = torch.tensor(duke["spectra"][:n],     dtype=torch.float32)
    tp   = torch.tensor(duke["temperature"][:n], dtype=torch.float32)

    # Normalise with training statistics
    sp_norm = (sp - stats["spec_mean"])  / stats["spec_std"]
    tp_norm = (tp - stats["temp_mean"]) / stats["temp_std"]

    all_hat = []
    with torch.no_grad():
        for i in range(0, n, 64):
            xh, _, _, _ = model(
                sp_norm[i:i+64].to(device),
                tp_norm[i:i+64].to(device),
            )
            all_hat.append(xh.cpu().numpy())

    x_hat_np  = np.concatenate(all_hat)
    x_true_np = sp_norm.numpy()
    return {"rmse_zero_shot": float(rmse_db(x_hat_np, x_true_np))}
...
def run_few_shot_finetune(
    ...
    """
    Few-shot fine-tuning: adapt the model on ``n_finetune`` Duke samples,
    then evaluate on the remainder.

    Returns
    -------
    dict with key ``rmse_few_shot``
    """
    import torch
    from evaluation.metrics import rmse_db
    from copy import deepcopy

    fs_data, te_data = split_duke(duke, n_fewshot=n_finetune)

    # Build tiny fine-tune loader
    sp_fs = torch.tensor(fs_data["spectra"],     dtype=torch.float32)
    tp_fs = torch.tensor(fs_data["temperature"], dtype=torch.float32)
    sp_norm = (sp_fs - stats["spec_mean"]) / stats["spec_std"]
    tp_norm = (tp_fs - stats["temp_mean"]) / stats["temp_std"]

    model_ft = deepcopy(model).to(device)
    opt = torch.optim.AdamW(model_ft.parameters(), lr=lr, weight_decay=1e-5)
    model_ft.train()
    for _ in range(epochs):
        perm = torch.randperm(len(sp_norm))
        for i in range(0, len(sp_norm), 16):
            idx = perm[i:i+16]
            xb = sp_norm[idx].to(device)
            tb = tp_norm[idx].to(device)
            _, _, loss, _ = model_ft(xb, tb)
            opt.zero_grad(); loss.backward(); opt.step()

    # Evaluate on test split
    sp_te = torch.tensor(te_data["spectra"],     dtype=torch.float32)
    tp_te = torch.tensor(te_data["temperature"], dtype=torch.float32)
    sp_te_norm = (sp_te - stats["spec_mean"]) / stats["spec_std"]
    tp_te_norm = (tp_te - stats["temp_mean"]) / stats["temp_std"]

    all_hat = []
    model_ft.eval()
    with torch.no_grad():
        for i in range(0, len(sp_te_norm), 64):
            xh, _, _, _ = model_ft(
                sp_te_norm[i:i+64].to(device),
                tp_te_norm[i:i+64].to(device),
            )
            all_hat.append(xh.cpu().numpy())

    x_hat_np  = np.concatenate(all_hat)
    x_true_np = sp_te_norm.numpy()
    return {"rmse_few_shot": float(rmse_db(x_hat_np, x_true_np))}
